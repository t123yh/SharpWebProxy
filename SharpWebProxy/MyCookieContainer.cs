...
namespace SharpWebProxy
{
    ...
    public class MyCookieContainer
    {
        ...
        public MyCookieContainer(int capacity)
        {
            if (capacity <= 0)
            {
        ...
        public MyCookieContainer(int capacity, int perDomainCapacity, int maxCookieSize) : this(capacity)
        {
            if (perDomainCapacity != int.MaxValue && (perDomainCapacity <= 0 || perDomainCapacity > capacity))
            {
                throw new ArgumentOutOfRangeException(nameof(perDomainCapacity));
            }
        ...
        public void Add(MyCookie cookie)
        {
            ...
            MyCookie new_cookie = cookie.Clone();
        ...
        internal void Add(MyCookie cookie, bool throwOnError)
        {
            PathList pathList;
            ...
            try
            {
                ...
                MyCookieCollection cookies;
                lock (pathList.SyncRoot)
                {
                    cookies = (MyCookieCollection) pathList[cookie.Path];

                    if (cookies == null)
                    {
                        cookies = new MyCookieCollection();
        ...
        private bool AgeCookies(string domain)
        {
            ...
            lock (m_domainTable.SyncRoot)
            {
                foreach (DictionaryEntry entry in m_domainTable)
                {
                    ...
                    lock (pathList.SyncRoot)
                    {
                        foreach (MyCookieCollection cc in pathList.Values)
                        {
                            ...
                            if (cc.Count > 0 && (tempUsed = cc.TimeStamp(MyCookieCollection.Stamp.Check)) < oldUsed)
                            {
                    ...
                    int min_count = Math.Min((int) (domain_count * remainingFraction),
                    ...
                    if (domain_count > min_count)
                    {
                        ...
                        lock (pathList.SyncRoot)
                        {
                            cookies = Array.CreateInstance(typeof(MyCookieCollection), pathList.Count);
                            stamps = Array.CreateInstance(typeof(DateTime), pathList.Count);
                            foreach (MyCookieCollection cc in pathList.Values)
                            {
                                stamps.SetValue(cc.TimeStamp(MyCookieCollection.Stamp.Check), itemp);
                        ...
                        {
                            MyCookieCollection cc = (MyCookieCollection) cookies.GetValue(i);
        ...
        private int ExpireCollection(MyCookieCollection cc)
        {
            lock (cc)
            {
        ...
        public void Add(MyCookieCollection cookies)
        {
            if (cookies == null)
            {
        ...
        internal bool IsLocalDomain(string host)
        {
            ...
            if (string.Compare(m_fqdnMyDomain, 0, host, dot, m_fqdnMyDomain.Length,
        ...
        public void Add(Uri uri, MyCookie cookie)
        {
            if (uri == null)
            {
        ...
        public void Add(Uri uri, MyCookieCollection cookies)
        {
            if (uri == null)
            {
                throw new ArgumentNullException(nameof(uri));
            ...
            foreach (MyCookie c in cookies)
            {
                MyCookie new_cookie = c.Clone();
                new_cookie.VerifySetDefaults(new_cookie.Variant, uri, isLocalDomain, m_fqdnMyDomain, true, true);
                Add(new_cookie, true);
            }
        }

        internal MyCookieCollection CookieCutter(Uri uri, string headerName, string setCookieHeader, bool isThrow)
        {
        ...
        public MyCookieCollection GetCookies(Uri uri)
        {
            if (uri == null)
            {
                throw new ArgumentNullException(nameof(uri));
            ...
            return InternalGetCookies(uri) ?? new MyCookieCollection();
        }

        internal MyCookieCollection InternalGetCookies(Uri uri)
        {
            if (m_count == 0)
            {
                return null;
            ...
            int port = uri.Port;
            MyCookieCollection cookies = null;
            ...
            BuildCookieCollectionFromDomainMatches(uri, isSecure, port, ref cookies,
                domainAttributeMatchAnyCookieVariant, false);
            if (domainAttributeMatchOnlyCookieVariantPlain != null)
            {
                BuildCookieCollectionFromDomainMatches(uri, isSecure, port, ref cookies,
                    domainAttributeMatchOnlyCookieVariantPlain, true);
            }

            return cookies;
        }

        private void BuildCookieCollectionFromDomainMatches(Uri uri, bool isSecure, int port,
            ref MyCookieCollection cookies, System.Collections.Generic.List<string> domainAttribute,
            ...
            for (int i = 0; i < domainAttribute.Count; i++)
            {
                ...
                lock (pathList.SyncRoot)
                {
                    ...
                    while (e.MoveNext())
                    {
                        ...
                        {
                            MyCookieCollection cc = (MyCookieCollection) e.Value;
                            cc.TimeStamp(MyCookieCollection.Stamp.Set);
        ...
        private void MergeUpdateCollections(ref MyCookieCollection destination, MyCookieCollection source, int port,
                ...
                for (int idx = 0; idx < source.Count; ++idx)
                {
                    ...
                    else
                    {
                        ...
                        if (to_add)
                        {
                            ...
                            if (destination == null)
                            {
                                destination = new MyCookieCollection();
                            }
        ...
        {
            MyCookieCollection cookies = InternalGetCookies(uri);
            if (cookies == null)
            {
            ...
            optCookie2 = cookies.IsOtherVersionSeen
                ? (MyCookie.SpecialAttributeLiteral +
                   CookieFields.VersionAttributeName +
                   MyCookie.EqualsLiteral +
                   MyCookie.MaxSupportedVersionString)
                : string.Empty;

            return builder.ToString();
        }

        public void SetCookies(Uri uri, string cookieHeader, bool throwIfError = false)
        {
            if (uri == null)
            {
                throw new ArgumentNullException(nameof(uri));
            ...
            CookieCutter(uri, null, cookieHeader, throwIfError); // No throw; just ignore
        }
    }
    ...
    public sealed class PathList
    {
        ...
        private readonly SortedList
            m_list = SortedList.Synchronized(
        ...
        internal int GetCookiesCount()
        {
            ...
            lock (SyncRoot)
            {
                foreach (MyCookieCollection cc in m_list.Values)
                {
                    count += cc.Count;
                }
            }
