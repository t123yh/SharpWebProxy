...
namespace SharpWebProxy
{
    ...
    [Serializable]
    public class MyCookieCollection : ICollection<MyCookie>, IReadOnlyCollection<MyCookie>, ICollection
    {
        internal enum Stamp
        {
            Check = 0,
        ...
        public MyCookie this[int index]
        {
            get
            {
                ...
                return m_list[index] as MyCookie;
            }
        }

        public MyCookie this[string name]
        {
            get
            {
                foreach (MyCookie c in m_list)
                {
        ...
        public void Add(MyCookieCollection cookies)
        {
            ...
            foreach (MyCookie cookie in cookies.m_list)
            {
                Add(cookie);
            }
        }
        ...
        public bool Contains(MyCookie cookie)
        {
            return IndexOf(cookie) >= 0;
        }

        public bool Remove(MyCookie cookie)
        {
            int idx = IndexOf(cookie);
            if (idx == -1)
            {
        ...
        public void CopyTo(MyCookie[] array, int index)
        {
...
*/
        ...
        internal
#endif
        int InternalAdd(MyCookie cookie, bool isStrict)
        {
            int ret = 1;
            if (isStrict)
            {
            ...
            if (cookie.Version != MyCookie.MaxSupportedVersion)
            {
        ...
        internal int IndexOf(MyCookie cookie)
        {
            int idx = 0;
            foreach (MyCookie c in m_list)
            {
        ...
        IEnumerator<MyCookie> IEnumerable<MyCookie>.GetEnumerator()
        {
            foreach (MyCookie cookie in m_list)
            {
                yield return cookie;
            }
        }
