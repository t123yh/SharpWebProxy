...
{
    [Serializable]
    public class MyCookieException : FormatException, ISerializable
    {
        public MyCookieException() : base()
        {
        }

        internal MyCookieException(string message) : base(message)
        {
        }

        internal MyCookieException(string message, Exception inner) : base(message, inner)
        {
        }

        protected MyCookieException(SerializationInfo serializationInfo, StreamingContext streamingContext)
    ...
    public sealed class MyCookie
    {
        ...
        private CookieVariant m_cookieVariant = CookieVariant.Default; // Do not rename (binary serialization)
...
*/
        ...
        static MyCookie()
        {
            Debug.Assert(MaxSupportedVersion.ToString(NumberFormatInfo.InvariantInfo)
        ...
        public MyCookie(string name, string value, string path, string domain)
...
*/
        ...
        internal
            ...
            MyCookie Clone()
        {
            MyCookie clonedCookie = new MyCookie(m_name, m_value);
            ...
            bool setDefault, bool shouldThrow)
        {
            ...
            if (m_name == null || m_name.Length == 0 || m_name[0] == '$' || m_name.IndexOfAny(ReservedToName) != -1)
            {
                if (shouldThrow)
                {
                    throw new MyCookieException("Cookie name invalid.");
                }
            ...
            // Check the value
            if (m_value == null ||
                (!(m_value.Length > 2 && m_value[0] == '\"' && m_value[m_value.Length - 1] == '\"') &&
                 m_value.IndexOfAny(ReservedToValue) != -1))
            {
                if (shouldThrow)
                {
                    throw new MyCookieException("Cookie value invalid.");
                }
                                ...
                                && (Comment.IndexOfAny(ReservedToValue) != -1))
            {
                if (shouldThrow)
                {
                    throw new MyCookieException("Cookie comment invalid.");
                }
                             ...
                             && (Path.IndexOfAny(ReservedToValue) != -1))
            {
                if (shouldThrow)
                {
                    throw new MyCookieException("Cookie path invalid.");
                }
            ...
            else
            {
                if (!m_domain_implicit)
                {
                    ...
                    if (!DomainCharsTest(domain))
                    {
                        if (shouldThrow)
                        {
                            throw new MyCookieException("Domain charset invalid.");
                        }
                    ...
                    if (domain[0] != '.')
                    {
                        if (!(variant == CookieVariant.Rfc2965 || variant == CookieVariant.Plain))
                        {
                            if (shouldThrow)
                            {
                                throw new MyCookieException("Cookie domain incorrect (doesn't begin with a dot).");
                            }
                    ...
                    else if (variant == CookieVariant.Plain)
                    {
                        ...
                        if (!IsDomainEqualToHost(domain, host))
                        {
                            if (host.Length <= domain.Length ||
                                (string.Compare(host, host.Length - domain.Length, domain, 0, domain.Length,
                                     StringComparison.OrdinalIgnoreCase) != 0))
                            {
                             ...
                             (string.Compare(host, host_dot, domain, 0, domain.Length,
                                  StringComparison.OrdinalIgnoreCase) != 0))
                    {
                ...
                if (!valid)
                {
                    if (shouldThrow)
                    {
                        throw new MyCookieException("Cookie Invalid");
                    }
...
*/
        ...
        {
            MyCookie other = comparand as MyCookie;
...
*/
            ...
            string ToServerString()
        {
            ...
            {
                result += SeparatorLiteral + CookieFields.CommentUrlAttributeName + EqualsLiteral + QuotesLiteral +
                          m_commentUri.ToString() + QuotesLiteral;
            }
            ...
            if (Expires != DateTime.MinValue)
            {
                ...
                result += SeparatorLiteral + CookieFields.MaxAgeAttributeName + EqualsLiteral +
            ...
            if (m_version > 0)
            {
                result += SeparatorLiteral + CookieFields.VersionAttributeName + EqualsLiteral +
